...
/**
 * 没有实际的，全是注释，省的以后用到再查了
 */
public class Comment {
	// @formatter:off
	/*
	 * 通常 Swing 不是线程安全的。除非另行说明，否则所有 Swing 组件及其相关类都必须保证在同一个线程（事件调度线程）中进行访问。
	 * 事件调度线程Event Dispatching Thread AWT 中有一个先进先出（FIFO）的事件队列（EventQueue）单例，
	 * 添加到该队列中的任务（Runnable）将按顺序逐一在同一线程中被执行，
	 * 该线程被称为 事件调度线程。Swing 组件也延用了该队列实例，
	 * 所有 Swing 组件的创建、修改、绘制、响应输入都必须要添加到该事件队列中执行。
	 * 组件注册的各种监听器，回调方法也是被添加到事件队列中执行，即所有监听器的回调方法中就处于事件调度线程，可以直接操作UI。 
	 * 添加任务到事件调度线程:
	 * EventQueue.invokeLater(Runnable doRun);
	 * SwingUtilities.invokeLater(Runnable doRun); 上面两者的区别: EventQueue 在 AWT
	 * 包中，SwingUtilities 在 Swing 包中，后者内部实际上是直接又调用了前者。
	 */
	/*
	 * 工作线程（SwingWorker）
	 * 事件调度线程负责 UI 的创建、修改、绘制 和 响应输入等，因此在该线程中不能执行耗时的操作（网络IO、密集计算处理），
	 * 否则将阻塞事件调度线程调度任何其他任务，造成应用程序对用户输入无反应。
	 * 对于耗时的任务，可以创建子线程在子线程中执行耗时操作，执行完后再提交任务到事件调度线程操作UI。
	 * 为此，Swing 提供了 SwingWorker 来处理耗时任务，并更好的支持了与事件调度线程之间的通信。
	 * SwingWorker 是一个抽象类，设计用于需要在后台线程中运行长时间运行任务的情况，
	 * 并可在完成后或者在处理过程中向 UI 提供数据并更新。
